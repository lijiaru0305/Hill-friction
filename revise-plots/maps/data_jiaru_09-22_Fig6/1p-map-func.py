...
rH = (mu/3.)**(1/3.)
krange = np.random.random(neps1)*2. + 2.
...
print(r"Max number of conjunctions corresponding to %6d orbital periods: %5d"%(tconjcrit,nconj))
...
nconjcrit = Parallel(n_jobs=ncore)(delayed(one_map)(k) for i in range(neps1))

np.savetxt(r"Nconjcrit_continuous_%1d.dat"%(int(-np.log10(mu))), [eps1range, nconjcrit])
